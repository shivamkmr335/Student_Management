...
import { HiOutlineBell, HiOutlineMail, HiOutlineMenu, HiOutlineUser, HiOutlineInboxIn , HiOutlineLogout } from 'react-icons/hi';
  ...
  return (
    ...
    <div className="w-screen bg-darkblue h-14 flex justify-between">
      ...
      <div className="flex items-center">
          ...
          <IoIosLogOut className="inline-block text-white w-6 h-6 mx-2 mr-4 cursor-pointer" onClick={()=>{
          logout();
          window.location.href = "/login";
          }} />
          <span className="inline-block w-7 h-7 mr-12" onClick={()=>setIsOpen(!isOpen)} >
          <Avatar size="small" border="square" imgUrl="https://designreset.com/cork/ltr/demo4/assets/img/profile-16.jpeg" className=" cursor-pointer " />
          </span>
      </div>
    </div>
...
const HeadlessUI: React.FC<Props2> = (props) => {
  ...
  return (
      ...
      <Transition.Root show={props.open} as={Fragment}>
        <Dialog open={props.open} onClose={props.onClose}>
          ...
          >
            <div className="fixed top-12 right-12 w-40 rounded z-20 p-2 bg-white transform font-myFont">           
                <button onClick={()=>props.onClose(false)} className="py-2 font-semibold text-gray-700 text-sm flex w-full flex-left"><HiOutlineUser    className="w-5 h-5 mx-2 text-gray-600" /><Link to="/rewards">Profile</Link></button>
                <button onClick={()=>props.onClose(false)} className="py-2 font-semibold text-gray-700 text-sm flex w-full text-left"><HiOutlineInboxIn className="w-5 h-5 mx-2 text-gray-600" /><Link to="/rewards">Inbox</Link></button>
                <div  onClick={()=>{ logout(); window.location.href = "/login";}} ><button onClick={()=>props.onClose(false)} className="py-2 font-semibold text-gray-700 text-sm flex w-full text-left" ><HiOutlineLogout  className="w-5 h-5 mx-2 text-gray-600" />Logout</button></div>   
                <Link to="/editAccount"><button onClick={()=>props.onClose(false)} className="py-2 font-semibold text-gray-700 text-sm flex w-full text-left"><FaUserEdit  className="w-5 h-5 mx-2 text-gray-600" />Edit Account</button></Link>   
            </div>
